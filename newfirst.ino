...
int indexUV=0;
float total, clEnergy, energy = 0, maxEnergy = 2000, min_energy;
float prevMin = 0;
...
void loop() 
{
  ...
  if(min_energy>=maxEnergy || prevMin>min_energy){
    total = 0;
    prevMin = 0;
  } else{
    prevMin = min_energy;
...
void sendMail(int ga){
    ...
    http.begin(client2, "http://maker.ifttt.com/trigger/uv_limit/with/key/bvZMIZ4hqr_S9APvvleOdD/?value1="+String(min_energy)+"&value2="+String(maxEnergy)+"&value3="+m); //HTTP
