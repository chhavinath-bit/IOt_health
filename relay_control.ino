...
float read_channel(float totalEnergy){
  ...
  Serial.println("energy from sensor1: ");
  Serial.println(totalEnergy);
  Serial.println("energy from sensor2: ");
  Serial.println(energys_2);
  Serial.println("statusCode2:");
  Serial.println(statusCode2);
  Serial.println("energy from sensor3: ");
  Serial.println(energys_3);
...
void relayControl(float totalEnergy){
  ...
  int maxEnergy = 500;
  int o = 0;
  float min_Energy=  read_channel(totalEnergy);
  Serial.println("minimum Energy: ");
  Serial.println(min_Energy);
  if(lampCmd==1 && totalEnergy<maxEnergy){
    digitalWrite(D3, HIGH);
    o = 1;
  }
  else if(totalEnergy>=maxEnergy || (statusCode==200 && lampCmd==0 && o==1)){
    digitalWrite(D3, LOW);
    String yn;
    if(statusCode==200){
      if(lampCmd==0){yn = "Yes";}
